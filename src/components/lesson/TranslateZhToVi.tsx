...
interface TranslateZhToViProps {
  sentence: Sentence;
  onAnswer: (answer: string, correct: boolean) => void;
}

const TranslateZhToVi = ({ sentence, onAnswer }: TranslateZhToViProps) => {
  ...
  const handleCheck = () => {
    if (checked) return;
    const correct = userAnswer.trim().toLowerCase() === sentence.vi.trim().toLowerCase() ||
                    userAnswer.trim().toLowerCase() === (sentence.correctAnswer || sentence.vi).trim().toLowerCase();
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
      ...
      <div className="mb-4">
        <textarea
          ...
          rows={4}
          className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-success dark:bg-gray-700 dark:border-gray-600 dark:text-white"
          disabled={checked}
        />
      </div>
      ...
      {checked && (
        <>
          ...
          >
            ...
            <div className="text-center">Đáp án đúng: {sentence.correctAnswer || sentence.vi}</div>
          </div>
          {!isCorrect && (
            <HintHelper
              sentence={sentence}
...
export default TranslateZhToVi;
